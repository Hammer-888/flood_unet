...
class DualStreamUNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        elif cfg.struct == "resunet":
            self.s1_stream = ResUnet(
                cfg,
                n_channels=cfg.in_channels,
                n_classes=out,
                topology=topology,
...
class UNet(nn.Module):
    def __init__(
        ...
        n_channels = cfg.in_channels if n_channels is None else n_channels
        n_classes = cfg.n_classes if n_classes is None else n_classes
        topology = cfg.topology if topology is None else topology
        ...
        for idx in range(n_layers):
            ...
            layer = Down(in_dim, out_dim, DoubleConv, CBAM, cfg.attention)
...
class Down(nn.Module):
    def __init__(self, in_ch, out_ch, conv_block, cbam_block, attention=False):
