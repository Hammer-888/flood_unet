...
class MultiThresholdMetric(object):
    def __init__(self, threshold):
        ...
        self._data_dims = (
            ...
            -3,
            -4,
        )  # For a B/W image, it should be [Thresh, B, C, H, W],
    ...
    def compute_oa(self):
        denom = (self.TP + self.FP + self.FN + self.TN).clamp(10e-05)
...
def false_pos(y_true: torch.Tensor, y_pred: torch.Tensor, dim=0):
    return torch.sum(y_true * (1.0 - torch.round(y_pred)), dim=dim)


def false_neg(y_true: torch.Tensor, y_pred: torch.Tensor, dim=0):
    return torch.sum((1.0 - y_true) * torch.round(y_pred), dim=dim)
...
def f1_score(gts: torch.Tensor, preds: torch.Tensor, dim=(-1, -2)):
    ...
    with torch.no_grad():
        ...
        f1 = 2.0 * recall_val * precision_val / denom
